...
function initApp(){
    products.forEach((value, key)=>{
        ...
        card.classList.add(value.category);
        card.innerHTML = `
            <div class="card-image w-full max-w-[400px] relative">
                <img src="images/${value.image}" alt="" class="card-img rounded-t-[14px] border-b-2 border-solid border-black object-cover w-full h-[370px] relative"/>

                <div class="category absolute top-0 left-0 p-[15px]">
                    <h2 class="text-[14px] font-[500] text-white bg-black py-[5px] px-[12px] rounded-[20px] uppercase">${value.category}</h2>
                </div>
            </div>
            ...
            <div class="card-content w-full flex justify-between items-center gap-2 px-3 pt-3 pb-4">
                <div class="desc ">
                    <h1 class="text-[18px] font-[600] text-black leading-[18px]">${value.name}</h1>
                    <p class="text-[14px] text-[#7d7d7d]">${value.title}</p>
...
//jquery for menu filtering ---------
$(document).ready(function(){

    $('.categ .categories').click(function(){
        ...
        if(filter == 'all'){
            ...
            $('.items .card').not('.'+filter).hide(200);
            $('.items .card').filter('.'+filter).show(400);
